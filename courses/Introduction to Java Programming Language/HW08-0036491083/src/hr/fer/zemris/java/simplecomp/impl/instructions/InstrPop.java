package hr.fer.zemris.java.simplecomp.impl.instructions;

import java.util.List;

import hr.fer.zemris.java.simplecomp.RegisterUtil;
import hr.fer.zemris.java.simplecomp.models.Computer;
import hr.fer.zemris.java.simplecomp.models.Instruction;
import hr.fer.zemris.java.simplecomp.models.InstructionArgument;

/**
 * This method implements CPU's method
 * pop which is used to pop some value from 
 * stack being placed in memory.
 * @author Leonardo Kokot
 * @version 1.0
 *
 */
public class InstrPop implements Instruction {

	/**
	 * This is index of register which value will be
	 * refresh with value from top of the stack.
	 */
	int value;
	
	/**
	 * This constructor creates instance of method pop.
	 * @param arguments Arguments based on which appropriate
	 * method pop is created.
	 * @throws IllegalArgumentException if illegal parameters are provided.
	 */
	public InstrPop(List<InstructionArgument> arguments) {
		if(arguments.size() != 1){
			throw new IllegalArgumentException("One argument expected.");
		}
		if(!arguments.get(0).isRegister()){
			throw new IllegalArgumentException("Type mismatch for argument.");
		}
		if(RegisterUtil.isIndirect((Integer)arguments.get(0).getValue())){
			throw new IllegalArgumentException("Indirect addressing is not supported "
					+ "in method pop.");
		}
		value = RegisterUtil.getRegisterIndex((Integer)arguments.get(0).getValue());
	}
	
	@Override
	public boolean execute(Computer computer) {
		int stackPointerRegValue = (int) computer.getRegisters().getRegisterValue(15);
		Object newValue = computer.getMemory().getLocation(stackPointerRegValue + 1);
		computer.getRegisters().setRegisterValue(15, stackPointerRegValue + 1);
		computer.getRegisters().setRegisterValue(value, newValue);
		return false;
	}

	

}
